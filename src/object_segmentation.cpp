...
Eigen::Vector3f fitLine(const Eigen::Vector2f &p1, const Eigen::Vector2f &p2) {
  ...
  line[0] = p2.y() - p1.y();
  line[1] = p1.x() - p2.x();
  line[2] = p2.x() * p1.y() - p1.x() * p2.y();
  line.normalize();

  return line;
}

float distanceToLine(const Eigen::Vector2f &point,
                     const Eigen::Vector3f &line) {
  ...
  for (int iter = 0; iter < ransac_max_iterations; ++iter) {
    ...
    for (size_t i = 0; i < cloud->points.size(); ++i) {
      ...
      if (distanceToLine(pt, line) < ransac_distance_threshold) {
        inliers->indices.push_back(projection_map.at(i));
      }
    }
  ...
  if (inliers->indices.size() > 0) {
    if (model_type == "line") {
      ...
      log_stream << "Circle model: center (" << coefficients->values[0] << ", "
                 << coefficients->values[1] << "), radius "
                 << coefficients->values[2];
    }
    LOG_INFO(log_stream.str());

    log_stream.str("");
    log_stream << model_type << " model inliers: " << inliers->indices.size();
    LOG_INFO(log_stream.str());
  } else {
    LOG_INFO("Could not estimate a " + model_type +
...
pcl::PointIndices::Ptr filterCircleInliers(
    ...
    const pcl::PointCloud<pcl::PointXYZRGBNormalRSD>::Ptr &original_cloud,
  ...
  LOG_INFO("- Circle inlier cloud size before filtering: " +
           std::to_string(inlier_cloud->points.size()));
  ...
  if (clusters.size() == 2) {
    ...
    for (const auto &idx : clusters[0].indices) {
      ...
      max_height_1 =
          std::max(max_height_1,
                   static_cast<double>(original_cloud->points[original_idx].z));
    }
    for (const auto &idx : clusters[1].indices) {
      size_t original_idx = projection_map.at(circle_inliers->indices[idx]);
      max_height_2 =
          std::max(max_height_2,
                   static_cast<double>(original_cloud->points[original_idx].z));
    }
    if (std::abs(max_height_1 - max_height_2) > circle_height_tolerance) {
      LOG_INFO("- Rejecting circle: height difference between clusters (" +
               ...
               ") exceeds tolerance (" +
               std::to_string(circle_height_tolerance) + ")");
      return filtered_inliers;
    }
  }
  ...
  Eigen::Vector3f circle_center(circle_coefficients->values[0],
                                circle_coefficients->values[1], 0.0f);
  for (const auto &idx : circle_inliers->indices) {
    size_t original_idx = projection_map.at(idx);
    const auto &point = original_cloud->points[original_idx];

    if (point.curvature < circle_curvature_threshold)
      continue;
    if (std::abs(point.r_min - circle_radius) > circle_radius_tolerance)
      continue;

    Eigen::Vector3f point_vector(point.x - circle_center.x(),
                                 point.y - circle_center.y(), 0.0f);
    ...
    if (std::min(angle, static_cast<float>(M_PI) - angle) >
...
pcl::PointIndices::Ptr filterLineInliers(
    ...
    const pcl::PointCloud<pcl::PointXYZRGBNormalRSD>::Ptr &original_cloud,
    const std::unordered_map<size_t, size_t> &projection_map,
    int line_min_cluster_size, int line_max_clusters,
  ...
  LOG_INFO("- Line inlier cloud size before filtering: " +
           std::to_string(inlier_cloud->points.size()));
  ...
  ec.setMaxClusterSize(line_max_clusters);
  ec.setSearchMethod(tree);
  ...
  if (clusters.size() > static_cast<size_t>(line_max_clusters)) {
    LOG_INFO("- Rejecting line: number of clusters (" +
             std::to_string(clusters.size()) + ") exceeds maximum allowed (" +
             std::to_string(line_max_clusters) + ")");
    return filtered_inliers;
  }
...
static bool areBinsNeighbors(const std::vector<int> &bin1,
                             const std::vector<int> &bin2,
                             int maxDistance = 1) {
  if (bin1.size() != bin2.size())
    return false;
  for (size_t i = 0; i < bin1.size(); ++i) {
    if (std::abs(bin1[i] - bin2[i]) > maxDistance)
      return false;
  }
  return true;
}
...
std::vector<HoughBin>
clusterLineModels(const std::vector<LineModel> &lineModels, double rhoThreshold,
  ...
  for (size_t i = 0; i < lineModels.size(); ++i) {
    ...
    for (size_t j = i + 1; j < lineModels.size(); ++j) {
      ...
      double theta_diff =
          std::min(std::abs(lineModels[i].theta - lineModels[j].theta),
    ...
    if (rhos.size() % 2 == 0) {
      newCluster.parameters = {(rhos[mid - 1] + rhos[mid]) / 2,
...
std::vector<HoughBin>
clusterCircleHoughSpace(const Eigen::Tensor<int, 3> &houghSpaceCircle,
                        double houghCenterXStep, double houghCenterYStep,
                        double houghRadiusStep, double minPt_x,
                        double minPt_y) {
  std::vector<HoughBin> clusters;
  ...
  std::sort(voteBins.begin(), voteBins.end(), [](const auto &a, const auto &b) {
  ...
  for (size_t m = 0; m < voteBins.size(); ++m) {
    ...
    for (size_t n = m + 1; n < voteBins.size(); ++n) {
      ...
      if (areBinsNeighbors({i, j, k}, {ni, nj, nk})) {
        ...
        newCluster.parameters[0] =
            (newCluster.parameters[0] * (totalVotes - neighborVotes) +
             (minPt_x + ni * houghCenterXStep) * neighborVotes) /
            totalVotes;
        newCluster.parameters[1] =
            (newCluster.parameters[1] * (totalVotes - neighborVotes) +
             (minPt_y + nj * houghCenterYStep) * neighborVotes) /
            totalVotes;
        newCluster.parameters[2] =
            (newCluster.parameters[2] * (totalVotes - neighborVotes) +
             (nk * houghRadiusStep) * neighborVotes) /
            totalVotes;
...
fitCylinderToCluster(
    const pcl::PointCloud<pcl::PointXYZRGBNormalRSD>::Ptr &cluster,
...
std::tuple<shape_msgs::msg::SolidPrimitive, geometry_msgs::msg::Pose>
fitBoxToCluster(const pcl::PointCloud<pcl::PointXYZRGBNormalRSD>::Ptr &cluster,
                double rho, double theta) {
  double phi = normalizeAngle(theta + M_PI_2);

  double along_x = std::cos(phi);
  double along_y = std::sin(phi);
  double perp_x = std::cos(theta);
...
std::vector<moveit_msgs::msg::CollisionObject> segmentObjects(
  ...
  for (const auto &cluster : cloud_clusters) {
    ...
    double hough_max_distance =
        std::sqrt(projected_x_range * projected_x_range +
                  projected_y_range * projected_y_range);

    std::vector<LineModel> lineModels;
    Eigen::Tensor<int, 3> hough_space_circle(
        hough_center_bins, hough_center_bins, hough_radius_bins);
    ...
    log_stream.str("");
    log_stream
        << "Parameter Spaces Initialization\n"
        << "  Line Model Storage:\n"
        << "    - Storing individual line models with rho, theta, votes, and "
           "inlier count\n"
        << "    - Line models will be clustered later based on similarity\n"
        << "  Circle Hough Space (3D):\n"
        << "    - Dimensions: " << hough_center_bins << " x "
        << hough_center_bins << " x " << hough_radius_bins << "\n"
        << "    - Center X: " << hough_center_bins
        << " bins (range: " << min_pt.x << " to " << max_pt.x
        << " m, step: " << hough_center_x_step << " m)\n"
        << "    - Center Y: " << hough_center_bins
        << " bins (range: " << min_pt.y << " to " << max_pt.y
        << " m, step: " << hough_center_y_step << " m)\n"
        << "    - Radius: " << hough_radius_bins << " bins (range: 0 to "
        << hough_max_radius << " m, step: " << hough_radius_step << " m)\n"
    ...
    for (int i = 0; i < num_iterations; ++i) {
      while (static_cast<int>(projected_cloud->points.size()) >
             inlier_threshold) {
        auto [line_inliers, line_coefficients] =
            fitLineRANSAC(projected_cloud, ransac_distance_threshold,
                          ransac_max_iterations, projection_map);
        auto [circle_inliers, circle_coefficients] = fitCircleRANSAC(
            projected_cloud, ransac_distance_threshold, ransac_max_iterations,
            hough_max_radius, projection_map);

        pcl::PointIndices::Ptr filtered_circle_inliers = filterCircleInliers(
            circle_inliers, cluster, circle_coefficients, projection_map,
            circle_min_cluster_size, circle_max_clusters,
            circle_height_tolerance, circle_curvature_threshold,
            circle_radius_tolerance, circle_normal_angle_threshold,
            circle_cluster_tolerance);

        logModelResults("Line", line_coefficients, line_inliers);

        pcl::PointIndices::Ptr filtered_line_inliers =
            filterLineInliers(line_inliers, cluster, projection_map,
                              line_min_cluster_size, line_max_clusters,
                              line_curvature_threshold, line_cluster_tolerance);

        std::vector<ValidModel> valid_models;
        if (static_cast<int>(filtered_circle_inliers->indices.size()) >
            inlier_threshold) {
          ValidModel circle_model;
          circle_model.type = "circle";
          circle_model.parameters = {circle_coefficients->values[0],
          ...
          LOG_INFO("  Inliers: " +
                   std::to_string(circle_model.inlier_indices.size()));
          ...
          LOG_INFO("  Inliers: " +
                   std::to_string(filtered_circle_inliers->indices.size()) +
                   " (Threshold: " + std::to_string(inlier_threshold) + ")");
        }
        if (static_cast<int>(filtered_line_inliers->indices.size()) >
            inlier_threshold) {
          ...
          LOG_INFO("  Inliers: " +
                   std::to_string(line_model.inlier_indices.size()));
          ...
          LOG_INFO("  Inliers: " +
                   std::to_string(filtered_line_inliers->indices.size()) +
                   " (Threshold: " + std::to_string(inlier_threshold) + ")");
        }
        ...
        if (!valid_models.empty()) {
          ...
          for (const auto &model : valid_models) {
            if (model.type == "circle") {
              circle_inliers =
                  std::max(circle_inliers, model.inlier_indices.size());
            } else if (model.type == "line") {
              line_inliers =
                  std::max(line_inliers, model.inlier_indices.size());
            }
          }

          // Might be broke
          const size_t c_size = circle_inliers;
          const size_t l_size = line_inliers;

          valid_models.erase(
              std::remove_if(
                  valid_models.begin(), valid_models.end(),
                  [c_size,
                   l_size](const auto &model) { // Capture the REAL variables
                    if (model.type == "circle")
                      return model.inlier_indices.size() < c_size;
                    if (model.type == "line")
                      return model.inlier_indices.size() < l_size;
                    return false;
                  }),
              valid_models.end());

          LOG_INFO("\n=== Additional Model Validation ===");
          LOG_INFO("  Max circle inliers: " + std::to_string(c_size));
          LOG_INFO("  Max line inliers: " + std::to_string(l_size));
          LOG_INFO("  Remaining valid models: " +
                   std::to_string(valid_models.size()));
        }

        LOG_INFO("");
        LOG_INFO("=== Model Validation Summary ===");
        LOG_INFO("  Total valid models: " +
                 std::to_string(valid_models.size()));
        LOG_INFO("  Circle models: " +
                 std::to_string(std::count_if(
                     valid_models.begin(), valid_models.end(),
                     [](const ValidModel &m) { return m.type == "circle"; })));
        LOG_INFO("  Line models: " +
                 std::to_string(std::count_if(
                     valid_models.begin(), valid_models.end(),
        ...
        for (const auto &model : valid_models) {
          inliers_to_remove.insert(model.inlier_indices.begin(),
                                   model.inlier_indices.end());
        }

        for (const auto &model : valid_models) {
          if (model.type == "circle") {
            int center_x_bin = static_cast<int>(
                (model.parameters[0] - min_pt.x) / hough_center_x_step);
            int center_y_bin = static_cast<int>(
                (model.parameters[1] - min_pt.y) / hough_center_y_step);
            int radius_bin =
        ...
        LOG_INFO("Removed " + std::to_string(inliers_to_remove.size()) +
                 "inliers. New projected cloud size: " +
                 std::to_string(projected_cloud->points.size()));
        LOG_INFO("");
      }
      // Log the reason for exiting the while loop
      if (static_cast<int>(projected_cloud->points.size()) <=
          inlier_threshold) {
        LOG_INFO(
            "Exiting RANSAC loop: Insufficient points remain. Points left: " +
            std::to_string(projected_cloud->points.size()) +
            ", Threshold: " + std::to_string(inlier_threshold));
      } else {
      ...
      LOG_INFO("Restored full point cloud for next iteration. Cloud size: " +
               std::to_string(projected_cloud->points.size()));
      LOG_INFO("Finishediteration " + std::to_string(i + 1) + " of " +
    ...
    std::vector<HoughBin> clusteredCircleModels = clusterCircleHoughSpace(
        hough_space_circle, hough_center_x_step, hough_center_y_step,
        hough_radius_step, min_pt.x, min_pt.y);

    LOG_INFO("Clustered models:");
    LOG_INFO("  Line clusters: " + std::to_string(clusteredLineModels.size()));
    LOG_INFO("  Circle clusters: " +
             std::to_string(clusteredCircleModels.size()));

    // Log the top clusters
    int topClustersToLog = 4;
    LOG_INFO("Top line clusters:");
    for (int i = 0; i < std::min(topClustersToLog,
                                 static_cast<int>(clusteredLineModels.size()));
         ++i) {
    ...
    for (int i = 0;
         i < std::min(topClustersToLog,
                      static_cast<int>(clusteredCircleModels.size()));
         ++i) {
      const auto &cluster = clusteredCircleModels[i];
      log_stream.str("");
      log_stream << "  Cluster " << i + 1 << ": Votes = " << cluster.votes
                 << ", Center = (" << cluster.parameters[0] << ", "
    ...
    if (!clusteredLineModels.empty() &&
        clusteredLineModels[0].votes > top_model_votes) {
      top_model_type = "line";
    ...
    if (!clusteredCircleModels.empty() &&
        clusteredCircleModels[0].votes > top_model_votes) {
    ...
    if (top_model_type == "circle") {
      ...
      auto [primitive, cylinder_pose] =
          fitCylinderToCluster(cluster, center_x, center_y, radius);
      ...
      std::ostringstream log_stream;
      log_stream << "Added cylinder collision object: id="
                 << collision_object.id
                 << ", height=" << primitive.dimensions[0]
                 << ", radius=" << primitive.dimensions[1] << ", position=("
                 << cylinder_pose.position.x << ", " << cylinder_pose.position.y
                 << ", " << cylinder_pose.position.z << ")" << ", yaw=" << yaw;
      LOG_INFO(log_stream.str());
      ...
      log_stream << "Added box collision object: id=" << collision_object.id
                 ...
                 << primitive.dimensions[1] << ", " << primitive.dimensions[2]
