...
GetPlanningSceneClient::GetPlanningSceneClient()
  ...
  client_ = create_client<mycobot_interfaces::srv::GetPlanningScene>(
...
GetPlanningSceneClient::call_service(
  ...
  // Prepare the request
  auto request =
      std::make_shared<mycobot_interfaces::srv::GetPlanningScene::Request>();
...
void GetPlanningSceneClient::log_response_info(
    const PlanningSceneResponse &response) {
  RCLCPP_INFO(get_logger(), "Response received. Success: %s",
              response.success ? "true" : "false");
  RCLCPP_INFO(get_logger(), "Target object ID: %s",
              response.target_object_id.c_str());
  RCLCPP_INFO(get_logger(), "Support surface ID: %s",
              response.support_surface_id.c_str());
  RCLCPP_INFO(get_logger(), "Number of collision objects: %zu",
              response.scene_world.collision_objects.size());
  RCLCPP_INFO(get_logger(), "Point cloud width: %d, height: %d",
              response.full_cloud.width, response.full_cloud.height);
  RCLCPP_INFO(get_logger(), "RGB image width: %d, height: %d",
  ...
  for (const auto &obj : response.scene_world.collision_objects) {
    ...
    RCLCPP_INFO(get_logger(), "  Number of primitives: %zu",
                obj.primitives.size());
    for (size_t i = 0; i < obj.primitives.size(); ++i) {
      ...
      RCLCPP_INFO(get_logger(), "      Dimensions: %zu",
                  prim.dimensions.size());
      for (size_t j = 0; j < prim.dimensions.size(); ++j) {
        RCLCPP_INFO(get_logger(), "        Dimension %zu: %.3f", j,
                    prim.dimensions[j]);
      }
      RCLCPP_INFO(get_logger(),
                  "      Pose: x=%.3f, y=%.3f, z=%.3f, qx=%.3f, qy=%.3f, "
                  "qz=%.3f, qw=%.3f",
                  prim_pose.position.x, prim_pose.position.y,
                  prim_pose.position.z, prim_pose.orientation.x,
                  prim_pose.orientation.y, prim_pose.orientation.z,
                  prim_pose.orientation.w);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  if (response.success) {
    ...
    RCLCPP_WARN(node->get_logger(),
                "Service response incomplete or invalid. Check "
                "get_planning_scene_server logs for the explanation.");
  }
