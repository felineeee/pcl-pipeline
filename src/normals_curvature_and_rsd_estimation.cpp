...
pcl::PointCloud<pcl::PointXYZRGBNormalRSD>::Ptr estimateNormalsCurvatureAndRSD(
    ...
    double rsd_radius) {
  LOG_INFO(
      "Starting normal, curvature, and RSD estimation. Input cloud size: " +
      std::to_string(input_cloud->size()) + " points");

  // Output: Point Cloud with Normal Vectors, Curvature Values, and RSD Values
  auto output_cloud =
      std::make_shared<pcl::PointCloud<pcl::PointXYZRGBNormalRSD>>();
  ...
  for (size_t i = 0; i < input_cloud->size(); ++i) {
    ...
    tree->nearestKSearch(input_cloud->points[i], k_neighbors, indices,
                         distances);
    ...
    int used_k = is_boundary ? std::min(static_cast<int>(indices.size()),
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr neighborhood(
    ...
    // The eigenvector corresponding to the smallest eigenvalue is an initial
    // normal estimate
    Eigen::Vector3f initial_normal = initial_pca.getEigenVectors().col(2);

    // Use Maximum Likelihood Estimation SAmple Consensus (MLESAC) to refine the
    // local plane estimate
    pcl::SampleConsensusModelPlane<pcl::PointXYZRGB>::Ptr model(
        new pcl::SampleConsensusModelPlane<pcl::PointXYZRGB>(neighborhood));
    ...
    float mu = std::accumulate(distances.begin(), distances.end(), 0.0f) /
               distances.size();

    // Compute weighted mean
    for (size_t j = 0; j < indices.size(); ++j) {
      Eigen::Vector3f pt = neighborhood->points[j].getVector3fMap();
      // Assign weights to neighbor points based on their distance to the
      // estimated local plane
      float weight =
          (std::find(inliers.begin(), inliers.end(), j) != inliers.end())
              ? 1.0f
              : std::exp(-(distances[j] * distances[j]) / (mu * mu));
    ...
    // The eigenvector corresponding to the smallest eigenvalue is the final
    // normal estimate
    ...
    // Compute curvature using the formula: γp = λ₀ / (λ₀ + λ₁ + λ₂) where λ₀ ≤
    // λ₁ ≤ λ₂
  ...
  pcl::PointCloud<pcl::PrincipalRadiiRSD>::Ptr rsd_features(
  ...
  ss << "Sample of output data structure (showing first " << sample_size
     << " points):\n";
  ss << std::left << std::setw(6) << "Point" << std::setw(8) << "X"
     << std::setw(8) << "Y" << std::setw(8) << "Z" << std::setw(4) << "R"
     << std::setw(4) << "G" << std::setw(4) << "B" << std::setw(10)
     << "Normal X" << std::setw(10) << "Normal Y" << std::setw(10) << "Normal Z"
     << std::setw(11) << "Curvature" << std::setw(8) << "RSD Min"
     << std::setw(8) << "RSD Max" << "\n";

  for (int i = 0; i < sample_size; ++i) {
    const auto &point = output_cloud->points[i];
    ss << std::setw(6) << i << std::setw(8) << point.x << std::setw(8)
       << point.y << std::setw(8) << point.z << std::setw(4)
       << static_cast<int>(point.r) << std::setw(4) << static_cast<int>(point.g)
       << std::setw(4) << static_cast<int>(point.b) << std::setw(10)
       << point.normal_x << std::setw(10) << point.normal_y << std::setw(10)
       << point.normal_z << std::setw(11) << point.curvature << std::setw(8)
       << point.r_min << std::setw(8) << point.r_max << "\n";
  }

  LOG_INFO(
      "Normal, curvature, and RSD estimation completed. Output cloud size: " +
      std::to_string(output_cloud->size()) + " points");
