...
#include "project_mtc_pickplace_pointcloud/get_planning_scene_client.h"
...
MTCTaskNode::MTCTaskNode(const rclcpp::NodeOptions &options)
  ...
  auto declare_parameter = [this](const std::string &name,
  ...
  declare_parameter("max_solutions", 25,
                    "Maximum number of solutions to compute");

  // Controller parameters
  declare_parameter(
      "controller_names",
      std::vector<std::string>{"arm_controller", "grip_action_controller"},
      "Names of the controllers to use");

  // Robot configuration parameters
  declare_parameter("arm_group_name", "arm",
                    "Name of the arm group in the SRDF");
  declare_parameter("gripper_group_name", "gripper",
                    "Name of the gripper group in the SRDF");
  declare_parameter("gripper_frame", "link6_flange",
                    "Name of the gripper frame");
  declare_parameter("gripper_open_pose", "open",
                    "Name of the gripper open pose");
  declare_parameter("gripper_close_pose", "half_closed",
                    "Name of the gripper closed pose");
  ...
  // Object parameters
  declare_parameter("object_name", "object",
                    "Name of the object to be manipulated");
  declare_parameter("object_type", "cylinder",
                    "Type of the object to be manipulated");
  declare_parameter("object_reference_frame", "base_link",
                    "Reference frame for the object");
  declare_parameter("object_dimensions", std::vector<double>{0.35, 0.0125},
                    "Dimensions of the object [height, radius]");
  declare_parameter("object_pose",
                    std::vector<double>{0.22, 0.12, 0.0, 0.0, 0.0, 0.0},
                    "Initial pose of the object [x, y, z, roll, pitch, yaw]");

  // Grasp and place parameters
  declare_parameter("grasp_frame_transform",
                    std::vector<double>{0.0, 0.0, 0.096, 1.5708, 0.0, 0.0},
                    "Transform from gripper frame to grasp frame [x, y, z, "
                    "roll, pitch, yaw]");
  declare_parameter(
      "place_pose", std::vector<double>{-0.183, -0.14, 0.0, 0.0, 0.0, 0.0},
      "Pose where the object should be placed [x, y, z, roll, pitch, yaw]");

  // Motion planning parameters
  declare_parameter("approach_object_min_dist", 0.0015,
                    "Minimum approach distance to the object");
  declare_parameter("approach_object_max_dist", 0.3,
                    "Maximum approach distance to the object");
  declare_parameter("lift_object_min_dist", 0.005,
                    "Minimum lift distance for the object");
  declare_parameter("lift_object_max_dist", 0.3,
                    "Maximum lift distance for the object");
  declare_parameter("lower_object_min_dist", 0.005,
                    "Minimum distance for lowering object");
  declare_parameter("lower_object_max_dist", 0.4,
                    "Maximum distance for lowering object");

  // Timeout parameters
  declare_parameter("move_to_pick_timeout", 10.0,
                    "Timeout for move to pick stage (seconds)");
  declare_parameter("move_to_place_timeout", 10.0,
                    "Timeout for move to place stage (seconds)");

  // Grasp generation parameters
  declare_parameter("grasp_pose_angle_delta", 0.1309,
                    "Angular resolution for sampling grasp poses (radians)");
  declare_parameter("grasp_pose_max_ik_solutions", 10,
                    "Maximum number of IK solutions for grasp pose generation");
  declare_parameter("grasp_pose_min_solution_distance", 0.8,
                    "Minimum distance in joint-space units between IK "
                    "solutions for grasp pose");

  // Place generation parameters
  declare_parameter("place_pose_max_ik_solutions", 10,
                    "Maximum number of IK solutions for place pose generation");

  // Cartesian planner parameters
  declare_parameter("cartesian_max_velocity_scaling", 1.0,
                    "Max velocity scaling factor for Cartesian planner");
  declare_parameter("cartesian_max_acceleration_scaling", 1.0,
                    "Max acceleration scaling factor for Cartesian planner");
  declare_parameter("cartesian_step_size", 0.00025,
                    "Step size for Cartesian planner");

  // Direction vector parameters
  declare_parameter("approach_object_direction_z", 1.0,
                    "Z component of approach object direction vector");
  declare_parameter("lift_object_direction_z", 1.0,
                    "Z component of lift object direction vector");
  declare_parameter("lower_object_direction_z", -1.0,
                    "Z component of lower object direction vector");
  declare_parameter("retreat_direction_z", -1.0,
                    "Z component of retreat direction vector");

  // Other parameters
  declare_parameter("place_pose_z_offset_factor", 0.5,
                    "Factor to multiply object height for place pose Z offset");
  declare_parameter("retreat_min_distance", 0.025,
                    "Minimum distance for retreat motion");
  declare_parameter("retreat_max_distance", 0.25,
                    "Maximum distance for retreat motion");

  RCLCPP_INFO(this->get_logger(),
              "All parameters have been declared with descriptions");
...
/**
 * @brief Update the target object parameters after the service call to get
 * planning scene
 */
void MTCTaskNode::updateObjectParameters(
    const moveit_msgs::msg::CollisionObject &collision_object) {
  // Update object_name
  this->set_parameter(rclcpp::Parameter("object_name", collision_object.id));
  RCLCPP_INFO(this->get_logger(), "Updated object_name: '%s'",
  ...
  if (!collision_object.primitives.empty()) {
    ...
    if (collision_object.primitives[0].type ==
        shape_msgs::msg::SolidPrimitive::CYLINDER) {
      object_type = "cylinder";
      dimensions = {
          collision_object.primitives[0]
              .dimensions[shape_msgs::msg::SolidPrimitive::CYLINDER_HEIGHT],
          collision_object.primitives[0]
              .dimensions[shape_msgs::msg::SolidPrimitive::CYLINDER_RADIUS]};
    } else if (collision_object.primitives[0].type ==
               shape_msgs::msg::SolidPrimitive::BOX) {
      object_type = "box";
      dimensions = {collision_object.primitives[0]
      ...
      RCLCPP_WARN(
          this->get_logger(),
          "Unsupported object type. Only cylinders and boxes are supported.");
      return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Updated object_type: '%s'",
                object_type.c_str());
...
void MTCTaskNode::setupPlanningScene() {
  ...
  auto object_dimensions =
      this->get_parameter("object_dimensions").as_double_array();
  auto object_pose_param = this->get_parameter("object_pose").as_double_array();
  auto object_reference_frame =
      this->get_parameter("object_reference_frame").as_string();
  ...
  RCLCPP_INFO(this->get_logger(), "  Dimensions: [%s]",
              std::accumulate(std::next(object_dimensions.begin()),
                              object_dimensions.end(),
  ...
  RCLCPP_INFO(this->get_logger(), "  Reference frame: %s",
              object_reference_frame.c_str());

  RCLCPP_INFO(this->get_logger(),
              "Sending GetPlanningScene service request...");

  // Call the service
  auto response =
      planning_scene_client->call_service(object_type, object_dimensions);

  RCLCPP_INFO(this->get_logger(),
              "Service call to the GetPlanningScene service completed.");
  ...
  RCLCPP_INFO(this->get_logger(),
              "Applying collision objects from service response...");
  if (!psi.applyCollisionObjects(scene_world_.collision_objects)) {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to add collision objects from service response");
  } else {
    RCLCPP_INFO(this->get_logger(),
                "Successfully added %zu collision objects from service "
                "response to the planning scene",
                scene_world_.collision_objects.size());
  }

  // Find the target object in the collision objects and update parameters
  RCLCPP_INFO(this->get_logger(),
              "Received target_object_id from service: '%s'",
...
void MTCTaskNode::doTask() {
  ...
  try {
    ...
    RCLCPP_ERROR(this->get_logger(), "Task initialization failed: %s",
                 e.what());
    return;
  }
  ...
  if (execute) {
    ...
    if (result.val != moveit_msgs::msg::MoveItErrorCodes::SUCCESS) {
      RCLCPP_ERROR(this->get_logger(),
                   "Task execution failed with error code: %d", result.val);
      return;
    }
...
mtc::Task MTCTaskNode::createTask() {
  ...
  auto gripper_group_name =
      this->get_parameter("gripper_group_name").as_string();
  ...
  auto gripper_close_pose =
      this->get_parameter("gripper_close_pose").as_string();

  // Frame parameters
  auto world_frame = this->get_parameter("world_frame").as_string();

  // Controller parameters
  auto controller_names =
      this->get_parameter("controller_names").as_string_array();
  ...
  // Grasp and place parameters
  auto grasp_frame_transform =
      this->get_parameter("grasp_frame_transform").as_double_array();
  auto place_pose = this->get_parameter("place_pose").as_double_array();

  // Motion planning parameters
  auto approach_object_min_dist =
      this->get_parameter("approach_object_min_dist").as_double();
  auto approach_object_max_dist =
      this->get_parameter("approach_object_max_dist").as_double();
  auto lift_object_min_dist =
      this->get_parameter("lift_object_min_dist").as_double();
  auto lift_object_max_dist =
      this->get_parameter("lift_object_max_dist").as_double();
  auto lower_object_min_dist =
      this->get_parameter("lower_object_min_dist").as_double();
  auto lower_object_max_dist =
      this->get_parameter("lower_object_max_dist").as_double();

  // Timeout parameters
  auto move_to_pick_timeout =
      this->get_parameter("move_to_pick_timeout").as_double();
  auto move_to_place_timeout =
      this->get_parameter("move_to_place_timeout").as_double();

  // Grasp generation parameters
  auto grasp_pose_angle_delta =
      this->get_parameter("grasp_pose_angle_delta").as_double();
  auto grasp_pose_max_ik_solutions =
      this->get_parameter("grasp_pose_max_ik_solutions").as_int();
  auto grasp_pose_min_solution_distance =
      this->get_parameter("grasp_pose_min_solution_distance").as_double();

  // Place generation parameters
  auto place_pose_max_ik_solutions =
      this->get_parameter("place_pose_max_ik_solutions").as_int();

  // Cartesian planner parameters
  auto cartesian_max_velocity_scaling =
      this->get_parameter("cartesian_max_velocity_scaling").as_double();
  auto cartesian_max_acceleration_scaling =
      this->get_parameter("cartesian_max_acceleration_scaling").as_double();
  auto cartesian_step_size =
      this->get_parameter("cartesian_step_size").as_double();

  // Direction vector parameters
  auto approach_object_direction_z =
      this->get_parameter("approach_object_direction_z").as_double();
  auto lift_object_direction_z =
      this->get_parameter("lift_object_direction_z").as_double();
  auto lower_object_direction_z =
      this->get_parameter("lower_object_direction_z").as_double();
  auto retreat_direction_z =
      this->get_parameter("retreat_direction_z").as_double();

  // Other parameters
  auto place_pose_z_offset_factor =
      this->get_parameter("place_pose_z_offset_factor").as_double();
  auto retreat_min_distance =
      this->get_parameter("retreat_min_distance").as_double();
  auto retreat_max_distance =
      this->get_parameter("retreat_max_distance").as_double();
  ...
  RCLCPP_INFO(this->get_logger(),
              "Joint Interpolation planner created for the gripper group");

  // Cartesian planner
  auto cartesian_planner = std::make_shared<mtc::solvers::CartesianPath>();
  cartesian_planner->setMaxVelocityScalingFactor(
      cartesian_max_velocity_scaling);
  cartesian_planner->setMaxAccelerationScalingFactor(
      cartesian_max_acceleration_scaling);
  ...
  task.setProperty("ik_frame",
                   gripper_frame); // The frame for inverse kinematics
   ...
   *                                                  *
   ***************************************************/
  // Pointer to store the current state (will be used during the grasp pose
  // generation stage)
   ...
   *                                                  *
   ***************************************************/
  // This stage is responsible for opening the robot's gripper in preparation
  // for picking up an object in the pick-and-place task.
  ...
  stage_open_gripper->properties().set(
      "trajectory_execution_info",
      mtc::TrajectoryExecutionInfo().set__controller_names(controller_names));
  ...
  // Create a pointer for the stage that will attach the object (to be used
  // later) By declaring it at the top level of the function, it can be accessed
  // throughout the entire task creation process. This allows different parts of
  // the code to use and modify this pointer.
   ...
   ***************************************************/
  {
    ...
    // This container will hold stages (in order) that will accomplish the
    // picking action
...
---- *               Approach Object                    *
     ***************************************************/
    {
      // Create a stage for moving the gripper close to the object before trying
      // to grab it. We are doing a movement that is relative to our current
      // position. Cartesian planner will move the gripper in a straight line
      ...
      stage->properties().set(
          "marker_ns",
          "approach_object"); // Namespace for visualization markers
      stage->properties().set("link",
                              gripper_frame); // The link to move (end effector)
      stage->properties().set(
          "trajectory_execution_info",
          mtc::TrajectoryExecutionInfo().set__controller_names(
              controller_names));
      stage->properties().configureInitFrom(
          mtc::Stage::PARENT, {"group"}); // Inherit the 'group' property
      stage->setMinMaxDistance(approach_object_min_dist,
                               approach_object_max_dist);

      // Define the direction that we want the gripper to move (i.e. z
      // direction) from the gripper frame
      geometry_msgs::msg::Vector3Stamped vec;
      vec.header.frame_id = gripper_frame; // Set the frame for the vector
      vec.vector.z =
          approach_object_direction_z; // Set the direction (in this case, along
                                       // the z-axis of the gripper frame)
...
---- *               Generate Grasp Pose               *
     ***************************************************/
    {
      ...
      // This stage is a generator stage because it doesn't need information
      // from stages before or after it. When generating solutions, MTC will try
      // to grab the object from many different orientations.
      // Sample grasp pose candidates in angle increments around the z-axis of
      // the object
      ...
      stage->setAngleDelta(
          grasp_pose_angle_delta); //  Angular resolution for sampling grasp
                                   //  poses around the object
      stage->setMonitoredStage(
          current_state_ptr); // Ensure grasp poses are valid given the initial
      ...
      wrapper->setIKFrame(vectorToEigen(grasp_frame_transform),
                          gripper_frame); // Transform from gripper frame to
...
---- *            Allow Collision (gripper,  object)   *
     ***************************************************/
    {
      // Modify planning scene (w/o altering the robot's pose) to allow touching
      // the object for picking
...
---- *       Allow collision (object,  support_surface)        *
     ***************************************************/
    {
      // Allows the planner to generate valid trajectories where the object
      // remains in contact with the support surface until it's lifted.
      auto stage = std::make_unique<mtc::stages::ModifyPlanningScene>(
          "allow collision (object,support_surface)");
...
---- *               Attach Object                     *
     ***************************************************/
    {
      ...
      stage->attachObject(object_name,
                          gripper_frame); // attach object to gripper_frame
...
---- *       Forbid collision (object, surface)*       *
     ***************************************************/
    {
      ...
      // This is important after the object has been lifted to ensure it doesn't
      // accidentally collide with the surface during subsequent movements.
      auto stage = std::make_unique<mtc::stages::ModifyPlanningScene>(
    ...
    // This serial container contains all the sequential steps we've created for
   ...
   *****************************************************/
  {
    ...
    // transport In other words, this stage plans the motion that transports the
    // object from where it was picked up to where it will be placed.
...
---- *          Generate Place Pose                       *
     *****************************************************/
    {
      ...
      target_pose_msg.pose.position.z +=
          place_pose_z_offset_factor * object_dimensions[0];
      stage->setPose(target_pose_msg);
      stage->setMonitoredStage(
          attach_object_stage); // hook into successful pick solutions
...
---- *          Detach Object                             *
     *****************************************************/
    {
      // Update the planning scene to reflect that the object is no longer
...
---- *          Retreat Motion                            *
  ...
  {
    auto stage =
        std::make_unique<mtc::stages::MoveTo>("move home", ompl_planner_arm);
    stage->properties().set(
        "trajectory_execution_info",
        mtc::TrajectoryExecutionInfo().set__controller_names(controller_names));
    stage->properties().configureInitFrom(mtc::Stage::PARENT, {"group"});
  ...
  try {
    ...
    try {
      ...
      RCLCPP_INFO(mtc_task_node->get_logger(),
                  "Task execution completed. Keeping node alive for "
                  "visualization. Press Ctrl+C to exit.");

      // Keep the node running until Ctrl+C is pressed
      executor.spin();
    } catch (const std::runtime_error &e) {
      RCLCPP_ERROR(mtc_task_node->get_logger(), "Runtime error occurred: %s",
                   e.what());
      ret = 1;
    } catch (const std::exception &e) {
      RCLCPP_ERROR(mtc_task_node->get_logger(), "An error occurred: %s",
                   e.what());
      ret = 1;
    }
  } catch (const std::exception &e) {
    RCLCPP_ERROR(rclcpp::get_logger("main"), "Error during node setup: %s",
                 e.what());
    ret = 1;
  }
