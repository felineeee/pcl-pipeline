...
std::vector<pcl::PointCloud<pcl::PointXYZRGBNormalRSD>::Ptr> extractClusters(
  ...
  LOG_INFO("Starting cluster extraction. Input cloud size: " +
           std::to_string(input_cloud->size()) + " points");

  // Create a KdTree object for the search method of the extraction
  auto tree =
      std::make_shared<pcl::search::KdTree<pcl::PointXYZRGBNormalRSD>>();
  ...
  pcl::RegionGrowing<pcl::PointXYZRGBNormalRSD, pcl::Normal> reg;
  ...
  reg.setSmoothnessThreshold(smoothness_threshold / 180.0f *
  ...
  const std::size_t sample_size =
      std::min(static_cast<std::size_t>(10), total_clusters);
  std::stringstream ss;
  ss << "Sample of output data structure (showing " << sample_size << " out of "
     << total_clusters << " clusters):\n";
  ss << std::left << std::setw(8) << "Cluster" << std::setw(10) << "Size"
     << std::setw(10) << "X" << std::setw(10) << "Y" << std::setw(10) << "Z"
     << std::setw(10) << "R" << std::setw(10) << "G" << std::setw(10) << "B"
     << std::setw(10) << "MinCurv" << std::setw(10) << "20Curv" << std::setw(10)
     << "AvgCurv" << std::setw(10) << "MedCurv" << std::setw(10) << "80Curv"
     << std::setw(10) << "MaxCurv" << std::setw(10) << "RMin" << std::setw(10)
     << "RMax" << "\n";
  ...
  for (std::size_t i = 0; i < sample_size; ++i) {
    ...
    float percentile_20_curvature =
        curvatures[static_cast<size_t>(curvatures.size() * 0.2)];
    float median_curvature = curvatures[curvatures.size() / 2];
    float percentile_80_curvature =
        curvatures[static_cast<size_t>(curvatures.size() * 0.8)];

    ss << std::setw(8) << i << std::setw(10) << cluster->size() << std::fixed
       << std::setprecision(2) << std::setw(10) << centroid[0] << std::setw(10)
       << centroid[1] << std::setw(10) << centroid[2] << std::setw(10)
       << std::round(avg_r) << std::setw(10) << std::round(avg_g)
       << std::setw(10) << std::round(avg_b) << std::fixed
       << std::setprecision(3) << std::setw(10) << min_curvature
       << std::setw(10) << percentile_20_curvature << std::setw(10)
       << avg_curvature << std::setw(10) << median_curvature << std::setw(10)
       << percentile_80_curvature << std::setw(10) << max_curvature
       << std::setw(10) << avg_rsd_min << std::setw(10) << avg_rsd_max << "\n";
  }
  ...
  ss << "\nKey:\n"
     ...
     << "RMin, RMax: Average minimum and maximum radii from Radius-Based "
        "Surface Descriptor (RSD)\n";

  LOG_INFO("Cluster extraction completed. Number of clusters found: " +
